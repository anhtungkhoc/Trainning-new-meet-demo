...
import { getUserByUsername, saveUser, addNewUser, getAllRoles, saveUserZoom, getUserZoom, updateUserZoom } from "./UserService";
...
class UserEditorDialog extends Component {
  ...
  handleChange = (event, source) => {
    ...
    if (source === "meetingPassw") {
      this.setState({ meetingPassw: event.target.value });
    }
    this.setState({
  ...
  handleFormSubmit = () => {
    let { id } = this.state;
    let { meetingId, meetingPassw } = this.state;
    console.log("Id: " + id);
    console.log("MeetingId: " + meetingId);
    console.log("MeetingPassw: " + meetingPassw);
    if (id != 0 && id != null) {
      updateUserZoom(id, meetingId, meetingPassw)
    }
  ...
  componentWillMount() {
    ...
    getUserZoom(item.id).then(({ data }) => {
      this.setState({
        meetingId: data.meetingId,
        meetingPassw: data.meetingPassw
      })
    })
    this.setState(item);
  }
