...
public class RestWebinarController {
  ...
  @RequestMapping(value = "/saveAndChangeRole", method = RequestMethod.POST)
  ...
  @RequestMapping(value = "/duc/{code}", method = RequestMethod.GET)
  public ResponseEntity<WebinarDto> getByUserName(@PathVariable String code) {
    WebinarDto result = service.getWebinarByCode(code);
    return new ResponseEntity<WebinarDto>(result, (result != null) ? HttpStatus.OK : HttpStatus.BAD_REQUEST);
  }
  ...
  public ResponseEntity<List<WebinarDto>> findAll() {
    List<WebinarDto> results = service.findAll();
    return new ResponseEntity<List<WebinarDto>>(results, HttpStatus.OK);
  }
